...
- **Writing the exhaustive project documentation honestly.** One real gap surfaced while documenting this project: the GUI tool is the only one of the three scripts with no accompanying PyInstaller .spec file, so its exact packaged-build settings for the client couldn't be verified from what's in the repo, only inferred from how its CLI siblings were packaged.
